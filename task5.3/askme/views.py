...
def hot(request):
    if request.user.is_authenticated:
        ...
        navbarTemplate = 'inc/navbar_viewed.html'
    hot_questions = models.Question.objects.hot(quantity=25)
    best_members = random.sample(list(models.User.objects.all()), k=5)
    popular_tags = random.sample(list(models.Tag.objects.all()), k=12)
...
def vote_correct(request):
    ...
    user = request.user
    if user == answer.to_question.author:
        STATUS_CHOICES = [
            ('c', 'correct'),
            ('u', 'unknown')
        ]
        if is_correct == 'true':
            answer.status = STATUS_CHOICES[0][0]
        else:
            answer.status = STATUS_CHOICES[-1][0]
